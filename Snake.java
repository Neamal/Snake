...
//		splash screen
		...
		if (!start) {
			...
			win.drawString("Made by: Junhyung Yoon", Snake.getMaxWindowX()/2 - 180, Snake.getMaxWindowY() - 120);
			win.setFont(new Font("Arial", Font.ITALIC, 20));
			win.setColor(Color.BLACK);
...
//		starts the game
		if (GDV5.KeysTyped[KeyEvent.VK_ENTER]) {
			start = true;
			win.drawImage(image.background, 0, 0, Snake.getMaxWindowX(), Snake.getMaxWindowY(), null);
			s1.draw(win);
